...
def matches_per_season_charts(request):
    ...
    return render(request, "MyApp/matches.html", context={"data": json_data})
...
def wins_per_season_charts(request):
    ...
    data = dict(context)
    json_data = json.dumps(data)
    return render(request, "MyApp/winsperseason.html", context={"data": json_data})


@cache_page(CACHE_TTL)
def extra_runs(request):
    extraruns_by_team = Matches.objects.filter(season=2016).annotate(bowling_team=F("deliveries__bowling_team")).values(
        "bowling_team").annotate(extra_runs=Sum("deliveries__extra_runs"))
...
def extra_runs_charts(request):
    ...
    data = dict(context)
    json_data = json.dumps(data)
    return render(request, "MyApp/extraruns.html", context={"data": json_data})


@cache_page(CACHE_TTL)
def economy(request):
    top_economical_bowler = Matches.objects.filter(season=2015).annotate(bowler=F("deliveries__bowler")).values(
        "bowler").annotate(economy=ExpressionWrapper(Sum("deliveries__total_runs") / (Count("deliveries__ball") / 6),
                                                     output_field=DecimalField())).order_by("economy")[:5]
    top_economical_bowler = [{"bowler": query["bowler"], "economy": round(float(query["economy"]), 2)} for query in
...
def economy_charts(request):
    ...
    data = dict(context)
    json_data = json.dumps(data)
    return render(request, "MyApp/economy.html", context={"data": json_data})


@cache_page(CACHE_TTL)
def batting_average(request):
    top_batsmen = Matches.objects.filter(season=2012).annotate(batsman=F("deliveries__batsman")).values(
        "batsman").annotate(
        batsman_avg=ExpressionWrapper(Sum("deliveries__batsman_runs") / Count(F("deliveries__match_id"), distinct=True),
                                      output_field=FloatField())).order_by("-batsman_avg")[:5]
...
def batting_average_charts(request):
    ...
    data = dict(context)
    json_data = json.dumps(data)
    return render(request, "MyApp/batting_avg.html", context={"data": json_data})